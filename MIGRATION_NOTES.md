...
**Reported:** the page never left its loading spinner and Chrome's own hang detector fired. `/climbing/ticklist` (same `Stat`/store/filtering machinery, larger dataset) worked fine, which ruled out `Stat.ts`, `buildStatTree`, and `StatFilter.vue` as shared-code causes.

**First fix attempt (insufficient on its own, kept - see below):**
1. **`chartOpts`/`colors` are `markRaw()`'d in `addDynamicChart` before ever entering the reactive `charts.dynamic` array** (`useClimberAnalysisStore.ts`), since both are mutated directly by third-party code (Chart.js's own internal bookkeeping; `getPieChartData` growing `colors` as new labels are discovered) outside Vue's awareness - a reactive Proxy around either is a documented source of vue-chartjs update loops. Verified: `isReactive(chartOpts)`/`isReactive(colors)` both `false`, `isReactive(opts)` (the rest - `filters`, `hideChart`, `splitStat`, etc., which `SettingView.vue`'s `v-model`s need) stays `true`.
2. **Restored `onUpdated(() => { loading.value = false })`**, present in the original `ClimberAnalysis.vue` and dropped without justification during the store migration.

The user re-tested after this landed and the hang was still there, with a Chrome performance trace and a console log showing `Stat {..., values: Proxy(Array), ...}` - proof the actual root cause was upstream of anything chart-related.

**Real root cause, confirmed via that evidence:** `useClimberAnalysisStore.ts`'s `ascents` was declared `ref<ProcessedAscent[]>([])`, not `shallowRef`. Vue's `ref()` deep-converts whatever gets assigned to `.value` via its internal `reactive()` - so `ascents.value = rawAscents.map(preprocessAscent)` produced a fully reactive-Proxy-wrapped array, *every element of which* then becomes the root `Stat`'s own `.values` (`goDeeper()` does `this.values = rawValues`) and, via `increment()` pushing those same element references, every substat's `.values` too - reaching the whole tree despite `Stat` instances themselves being `markRaw`'d (`markRaw` only stops a *container* from being wrapped; it does nothing to unwrap a value that arrives from elsewhere already reactive). `useClimberTicklistStore.ts` never had this problem because it builds `allAscentsFlat` as a local `const`, never assigning it to any `ref` before passing it to `buildStatTree` - the key reason the ticklist page never hung.

**Fix:** `ascents` changed from `ref` to `shallowRef` in `useClimberAnalysisStore.ts`. Confirmed via `preprocessAscent` (`Utils.ts`) that this is sufficient on its own, with no matching fix needed in `SandboxAnalysis.vue`'s or `ClimbingImportAnalysis.vue`'s own local ascent refs: `preprocessAscent` builds its result via object-spread (`{ ...ascent, ... }`) into a brand-new plain object, which reads each property off whatever it's given (reactive or not) but always produces a genuinely non-reactive result - it launders any upstream reactivity before the array is ever assigned into this store's `ascents`.

**Verified two ways** (via `vite-node`, real scorecard data, 3,413 ascents):
- Before the fix: `isProxy(stats.value.values)` / `isReactive(...)` / same on `.values[0]` all `true`, exactly matching the user's `Proxy(Array)` console log. After: all `false`.
- Repeatedly iterating every field of a reactive-Proxy-wrapped ascent array vs. the identical plain array (simulating what 6 chart builds + a table render do): **~22x slower** (1857ms vs 85ms over 20 passes) - a large enough, real enough cost, compounded across every filter change and re-render, to plausibly read as a genuine browser hang rather than mere sluggishness.

**Confidence:** high - unlike the first attempt, this fix directly targets and eliminates the exact symptom (`Proxy(Array)`) the user observed, with a measured mechanism (22x slower field iteration) that scales the way a hang would. Still asks for a real-browser re-test before considering this fully closed, since this environment has no browser/Canvas rendering.
