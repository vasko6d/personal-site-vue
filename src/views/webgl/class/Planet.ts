...
export default class Planet {
  ...
  translationMatrix(time: number): Mat {
    return mv.mult(mv.translationMatrix(this.getPosition(time)), mv.scalarMatrix(this.size, this.size, this.size)) as Mat;
  }
  ...
  animate(gl: WebGLRenderingContext, loc: UniformLocations, light: Light, time: number, prevMat: Mat = mv.mat4()) {
  ...
  animateMoons(gl: WebGLRenderingContext, loc: UniformLocations, light: Light, time: number, prevMat: Mat) {
