...
const selectedAscents = computed<AscentLike[]>(() => {
  ...
  if (selectedDay.value) {
    ret = props.stat.get("date").get(toStatDateString(selectedDay.value)).values as unknown as AscentLike[];
  }
  return ret;
});
...
function createDataset(data: { x: Date; y: number }[], opts: { color: string; label: string }): TimeSeriesDataset {
  return {
    data: data,
...
const chartData = computed<ChartData<"line">>(() => {
  ...
  const data: Record<"sinceMax" | "max" | "avg" | "numMax" | "sinceNewMax", { x: Date; y: number }[]> = {
    sinceMax: [],
    max: [],
    avg: [],
    numMax: [],
...
<template>
  <div class="chart-ts bg1">
    ...
    <h2>
      Time Series
      <i @click="showOptions = !showOptions" class="fas icn fa-cog" :class="{ 'icn-a': showOptions }"></i>
    </h2>
