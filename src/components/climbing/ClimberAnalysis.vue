...
function createChart(chartType: string, statBase: string, opts: DynamicChartConfig["opts"]): Chart {
  ...
  switch (chartType) {
    ...
    case "pie": {
      ...
      plugins.legend = {
        display: !!opts.splitStat || (chartType === "pie" && dynamicChart.chartData.labels.length < 8),
      };
      break;
    }
    case "grade":
...
function addDynamicChart(chartType: string, statBase: string, opts: DynamicChartConfig["opts"] = {}) {
  ...
  <div id="climber-analysis">
    ...
    <div v-else>
      <div class="flex-row">
        ...
        <div class="chart bg1">
          <StatFilter :currentFilters="currentFilters" :stats="stats" @clearFilters="clearFilters" />
        </div>
      </div>
      ...
      <div v-if="showDynamicCharts">
        ...
        <div class="flex-row">
          <TimeSeriesChart
            v-if="
              currentFilteredStat && currentFilteredStat.values.length > 0 && initialized && tsIsOpen
            "
