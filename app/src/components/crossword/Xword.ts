...
export default class Xword {
  ...
  move(d: { r: number; c: number }, skipFilled = false, origCell?: { r: number; c: number } | false) {
  ...
  clearClue() {
    const clue = this.isHoriz ? this.across[this.getCell().acrossNum!]! : this.down[this.getCell().downNum!]!;
    for (const cell of clue.ctx) {
      this.clearCell(cell);
    }
    this.bulkUpdateClueFlags();
    ...
    for (const row of this.puzzle) {
      for (const cell of row) {
        ...
        stats.numTrulySolved +=
          !this.isInputColorCell(cell) || cell.wasAutoSolved || cell.wasShownError || cell.wasWrongCleared
            ? 0
            : 1;
      }
    }
