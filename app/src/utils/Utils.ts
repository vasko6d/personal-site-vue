...
export function getGradeChartData(
  ...
  gradeList.sort((a, b) => (gradeMap[String(a.name)] as number) - (gradeMap[String(b.name)] as number));
  ...
  if (ascents.length > 0) {
    ...
    for (const ascent of ascents) {
      ...
      if (opts.avgSamples && opts.avgSamples > 0) {
        ...
        t.run.avg =
          t.run.sampledGrades.reduce((a, b) => a + b, 0) / t.run.sampledGrades.length;
      }
...
export function updateValues(
  ...
  tsTracker.score =
    tsTracker.top.reduce((a, b) => a + b, 0) / Math.min(nTop, tsTracker.top.length);
}
...
export function updateSinceMax(
  ...
  const compGrade =
    comparisonGradeOpt !== undefined && comparisonGradeOpt > -1
      ? comparisonGradeOpt
      : t.run.max;
