...
const currentClue = computed(() => {
  ...
  const entry = num != null ? (isAcross ? xword.value.across[num] : xword.value.down[num]) : undefined;
          ...
          promises.push(
            new Promise((resolve2) => {
              fetch(`/json/xwords/clues/${props.xwordId}.json`).then((response) => {
                response.json().then((json: { across: Record<string, string>; down: Record<string, string> }) => {
