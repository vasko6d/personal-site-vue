...
export default class Galaxy {
  ...
  constructor(lightSource: Light, seedPoints: SeedPoints, minComplexity: number, maxComplexity: number) {
    this.vertices = [];
    this.normals = [];
    this.flatNormals = [];
    this.planets = [];
  ...
  addPlanet(complexity: number, size: number, material: Material, orbit: Orbit, name?: string) {
    this.planets.push(new Planet(size, material, orbit, this.complexityVertexMap[complexity]!, name));
  }
  ...
  addMoon(planetIndexPath: number[], complexity: number, size: number, material: Material, orbit: Orbit, name?: string) {
    let p = this.getPlanetByIndex(planetIndexPath[0]!);
    for (let i = 1; i < planetIndexPath.length; i++) {
      p = p.getMoonByIndex(planetIndexPath[i]!);
    }
    p.moons.push(new Planet(size / p.size, material, orbit, this.complexityVertexMap[complexity]!, name));
  }
